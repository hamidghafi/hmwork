...
%         V=[zeros(1,floor((N-1)/3)),1,zeros(1,floor(2*(N)/3))];
        ...
        for th=theta
            ...
            Pat=[Pat,abs(g(th))^2*abs(sin(th))];
        end
        P=sum(Pat)*.005/2;
        G=10*log10(2/P)+max(Patternt);
        figure(10+F)
        subplot(1,2,1)
        ...
        disp(['Max Gain for Linear antenna L = ',num2str(C),' lambda N = ',num2str(N),' : ',num2str(G)])
    end
        Imped=[Imped,J((N+1)/2)];
end
% figure(3)
